...
export default function NotificationProvider({ children }) {
  ...
  useEffect(() => {
    ...
    return () => {
      notificationRef.current?.classList.add("bottom-14", "opacity-0"); // eslint-disable-next-line
    ...
    <>
      ...
      {notification.value ? (
        <p
          ...
          className={
            backgroundColor +
            " rounded p-2 text-white fixed bottom-14 opacity-0 right-1/2 -translate-y-1/2 transition-all duration-150 ease-linear"
          }
        >
          {notification.value}
        </p>
